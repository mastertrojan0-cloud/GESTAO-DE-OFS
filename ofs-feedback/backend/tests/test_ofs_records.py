"""
test_ofs_records.py — Testes de CRUD de registros OFS.

Cenarios:
  1. Criar OFS valido -> 201 + codigo
  2. Criar OFS valido (tipo Negativo) -> 201
...
# ---------------------------------------------------------------------------
# 2. Criar OFS valido (tipo Negativo/Inseguro) -> 201
# ---------------------------------------------------------------------------

@pytest.mark.asyncio
async def test_create_ofs_negativo_success(client: AsyncClient, observador_headers, valid_ofs_negativo_payload):
    """Criacao de OFS (tipo Negativo (Inseguro)) com dados validos retorna 201."""
    resp = await client.post("/api/ofs-records", json=valid_ofs_negativo_payload, headers=observador_headers)
    assert resp.status_code == 201
    data = resp.json()
    assert data["tipo"] == "Negativo (Inseguro)"
...
@pytest.mark.asyncio
async def test_create_ofs_missing_required_fields(client: AsyncClient, observador_headers):
...
@pytest.mark.asyncio
async def test_create_ofs_nome_observado_too_short(client: AsyncClient, observador_headers, valid_ofs_payload):
    """nome_observado com menos de 3 chars retorna 422."""
    payload = {**valid_ofs_payload, "nome_observado": "AB"}
    resp = await client.post("/api/ofs-records", json=payload, headers=observador_headers)
    assert resp.status_code == 422


@pytest.mark.asyncio
async def test_create_ofs_comportamento_too_short(client: AsyncClient, observador_headers, valid_ofs_payload):
    """comportamento com menos de 3 chars (? min_length=3 no schema atual)."""
    payload = {**valid_ofs_payload, "comportamento": "AB"}
    resp = await client.post("/api/ofs-records", json=payload, headers=observador_headers)
    assert resp.status_code == 422
...
@pytest.mark.asyncio
async def test_create_ofs_future_date(client: AsyncClient, observador_headers, valid_ofs_payload):
    """Data no futuro nao e explicitamente validada no schema atual,
    mas o formato deve ser YYYY-MM-DD."""
    payload = {**valid_ofs_payload, "data": "2099-12-31"}
    resp = await client.post("/api/ofs-records", json=payload, headers=observador_headers)
...
@pytest.mark.asyncio
async def test_create_ofs_invalid_hour_format(client: AsyncClient, observador_headers, valid_ofs_payload):
    """Hora em formato invalido retorna 422."""
    payload = {**valid_ofs_payload, "hora": "14:30:00"}
    resp = await client.post("/api/ofs-records", json=payload, headers=observador_headers)
    assert resp.status_code == 422
...
@pytest.mark.asyncio
async def test_list_ofs_observador_sees_only_own(
    client: AsyncClient, observador_headers, valid_ofs_payload, gestor_headers
):
    """Observador ve apenas seus proprios registros."""
    # Criar um OFS como observador
    await client.post("/api/ofs-records", json=valid_ofs_payload, headers=observador_headers)
...
@pytest.mark.asyncio
async def test_list_ofs_gestor_sees_all(
    client: AsyncClient, observador_headers, valid_ofs_payload, gestor_headers
):
...
@pytest.mark.asyncio
async def test_get_ofs_by_id_owner(client: AsyncClient, observador_headers, valid_ofs_payload):
    """Dono do registro pode visualizar seu OFS."""
    create_resp = await client.post("/api/ofs-records", json=valid_ofs_payload, headers=observador_headers)
...
@pytest.mark.asyncio
async def test_get_ofs_other_user_as_observador(
    client: AsyncClient, observador_headers, gestor_headers, valid_ofs_payload
):
    """Observador tentando acessar OFS de outro usuario recebe 403."""
    # Gestor cria OFS
    create_resp = await client.post("/api/ofs-records", json=valid_ofs_payload, headers=gestor_headers)
    ofs_id = create_resp.json()["id"]
...
@pytest.mark.asyncio
async def test_edit_own_ofs_within_window(
    client: AsyncClient, observador_headers, valid_ofs_payload
):
...
@pytest.mark.asyncio
async def test_edit_ofs_optimistic_lock_conflict(
    client: AsyncClient, observador_headers, valid_ofs_payload
):
...
@pytest.mark.asyncio
async def test_cancel_ofs_as_gestor(
    client: AsyncClient, observador_headers, valid_ofs_payload, gestor_headers
):
...
@pytest.mark.asyncio
async def test_cancel_ofs_as_admin(
    client: AsyncClient, observador_headers, valid_ofs_payload, admin_headers
):
...
@pytest.mark.asyncio
async def test_cancel_ofs_short_reason(
    client: AsyncClient, observador_headers, valid_ofs_payload, gestor_headers
):
...
@pytest.mark.asyncio
async def test_cancel_ofs_as_observador(
    client: AsyncClient, observador_headers, valid_ofs_payload
):
...
@pytest.mark.asyncio
async def test_cancel_ofs_as_supervisor(
    client: AsyncClient, observador_headers, valid_ofs_payload, supervisor_headers
):
...
@pytest.mark.asyncio
async def test_restore_ofs_as_admin(
    client: AsyncClient, observador_headers, valid_ofs_payload, gestor_headers, admin_headers
):
...
@pytest.mark.asyncio
async def test_restore_ofs_as_gestor(
    client: AsyncClient, observador_headers, valid_ofs_payload, gestor_headers
):
...
@pytest.mark.asyncio
async def test_list_ofs_with_filters(
    client: AsyncClient, gestor_headers, valid_ofs_payload, valid_ofs_negativo_payload
):
    """Filtros de listagem funcionam corretamente."""
    await client.post("/api/ofs-records", json=valid_ofs_payload, headers=gestor_headers)
    await client.post("/api/ofs-records", json=valid_ofs_negativo_payload, headers=gestor_headers)
...
@pytest.mark.asyncio
async def test_list_ofs_pagination(
    client: AsyncClient, gestor_headers, valid_ofs_payload
):
...
@pytest.mark.asyncio
async def test_supervisor_g4s_cannot_see_sd_ofs(
    client: AsyncClient, observador_headers, valid_ofs_payload, supervisor_g4s_headers
):
